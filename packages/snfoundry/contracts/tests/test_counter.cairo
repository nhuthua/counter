...
//#[test]
fn test_decrement_counter_happy_path(){
...
fn test_decrement_counter_fail_path(){
    let init_counter: u32 = 0;
    let dispatcher = deploy_counter(init_counter);
...
fn test_reset_counter_sucess(){
    ...
    assert!(erc_20.balance_of(user) == 9000000000000000000, "Balance not deducted correctly");
    assert!(erc_20.balance_of(owner_address()) == 1000000000000000000, "Balance not increased correctly");

    println!("user balance: {}", erc_20.balance_of(user));
    println!("owner balance: {}", erc_20.balance_of(owner_address()));
}
