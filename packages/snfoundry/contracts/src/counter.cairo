...
pub mod CounterContract {
    ...
    impl CounterImpl of ICounter<ContractState> {
            ...
            fn decrease_counter(ref self: ContractState){
                let current_counter = self.counter.read();
                assert!(current_counter > 0, "The counter can't be negative"); // check and notify
                let new_counter = current_counter - 1;
                self.counter.write(new_counter);
                let event: CounterChanged = CounterChanged{
                    old_value: current_counter,
            ...
            fn reset_counter(ref self: ContractState){
                let payment_amount: u256 = 1000000000000000000; // 1 STRK
                let strk_token: ContractAddress = strk_address();  //dia chi
